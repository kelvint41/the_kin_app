...
// Color palette (10 stops, premultiplied alpha)
...
float fbmNoise(vec2 p, int octaves, float lacunarity, float persistence) {
    float value = 0.0;
    float amplitude = 0.5;
    float frequency = 1.0;
    float maxValue = 0.0;

    // Rotation matrix for domain warping between octaves
...
// 3D FBM - noise evolves over time without translation
float fbmNoise3D(vec3 p, int octaves, float lacunarity, float persistence) {
    float value = 0.0;
    float amplitude = 0.5;
    float frequency = 1.0;
    float maxValue = 0.0;

    // 3D rotation matrix for domain warping
    mat3 rot = mat3(
        0.8, 0.6, 0.0,
        -0.6, 0.8, 0.0,
        0.0, 0.0, 1.0
...
// Animated FBM using time as Z coordinate - noise evolves in place
float animatedFbm(vec2 p, float time, int octaves, float lacunarity, float persistence) {
    return fbmNoise3D(vec3(p, time * 0.3), octaves, lacunarity, persistence);
}
...
vec3 computeNormal(vec2 uv, float time, int octaves, float bumpStrength) {
    vec2 noiseCoord = uv * uNoiseScale;
    float eps = 0.01;

    float center = animatedFbm(noiseCoord, time, octaves, uLacunarity, uPersistence);
    float right = animatedFbm(noiseCoord + vec2(eps, 0.0), time, octaves, uLacunarity, uPersistence);
    float up = animatedFbm(noiseCoord + vec2(0.0, eps), time, octaves, uLacunarity, uPersistence);
