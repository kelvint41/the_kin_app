...
float turbulenceNoise(vec2 p, int octaves, float baseFreq) {
    float value = 0.0;
    float amplitude = 0.5;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        // Use abs() to create ridge-like features
...
// 3D Turbulence - noise evolves over time without translation
...
// Animated Turbulence using time as Z coordinate - noise evolves in place
...
vec3 computeNormal(vec2 uv, float time, int octaves, float bumpStrength) {
    ...
    float center = animatedTurbulence(noiseCoord, time, octaves, uBaseFrequency);
    float right = animatedTurbulence(noiseCoord + vec2(eps, 0.0), time, octaves, uBaseFrequency);
    float up = animatedTurbulence(noiseCoord + vec2(0.0, eps), time, octaves, uBaseFrequency);
