...
class VoronoiGradientShaderFill extends StatelessWidget {
  VoronoiGradientShaderFill({
    ...
    this.cache = false,
  }) : params = params ?? voronoiGradientDef.defaults;
  ...
  static Future<void> precacheShader() =>
      ShaderBuilder.precacheShader('packages/material_palette/shaders/voronoi_gradient.frag');

  void _setUniforms(FragmentShader shader, Size size, double time) {
    setShaderUniforms(shader, size, time, params, voronoiGradientDef.layout);
  }
  ...
  Widget build(BuildContext context) {
    return ShaderFill(
      width: width,
      height: height,
      shaderPath: 'packages/material_palette/shaders/voronoi_gradient.frag',
