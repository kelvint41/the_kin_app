...
class RadialVoronoiGradientShaderFill extends StatelessWidget {
  RadialVoronoiGradientShaderFill({
    ...
    this.cache = false,
  }) : params = params ?? radialVoronoiGradientDef.defaults;
  ...
  static Future<void> precacheShader() =>
      ShaderBuilder.precacheShader('packages/material_palette/shaders/radial_voronoi_gradient.frag');

  void _setUniforms(FragmentShader shader, Size size, double time) {
    setShaderUniforms(shader, size, time, params, radialVoronoiGradientDef.layout);
  }
  ...
  Widget build(BuildContext context) {
    return ShaderFill(
      width: width,
      height: height,
      shaderPath: 'packages/material_palette/shaders/radial_voronoi_gradient.frag',
