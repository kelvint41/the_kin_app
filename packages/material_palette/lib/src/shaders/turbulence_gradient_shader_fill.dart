...
class TurbulenceGradientShaderFill extends StatelessWidget {
  TurbulenceGradientShaderFill({
    ...
    this.cache = false,
  }) : params = params ?? turbulenceGradientDef.defaults;
  ...
  static Future<void> precacheShader() =>
      ShaderBuilder.precacheShader('packages/material_palette/shaders/turbulence_gradient.frag');

  void _setUniforms(FragmentShader shader, Size size, double time) {
    setShaderUniforms(shader, size, time, params, turbulenceGradientDef.layout);
  }
  ...
  Widget build(BuildContext context) {
    return ShaderFill(
      width: width,
      height: height,
      shaderPath: 'packages/material_palette/shaders/turbulence_gradient.frag',
