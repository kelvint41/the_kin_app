...
class VoronoiseGradientShaderFill extends StatelessWidget {
  VoronoiseGradientShaderFill({
    ...
    this.cache = false,
  }) : params = params ?? voronoiseGradientDef.defaults;
  ...
  static Future<void> precacheShader() =>
      ShaderBuilder.precacheShader('packages/material_palette/shaders/voronoise_gradient.frag');

  void _setUniforms(FragmentShader shader, Size size, double time) {
    setShaderUniforms(shader, size, time, params, voronoiseGradientDef.layout);
  }
  ...
  Widget build(BuildContext context) {
    return ShaderFill(
      width: width,
      height: height,
      shaderPath: 'packages/material_palette/shaders/voronoise_gradient.frag',
