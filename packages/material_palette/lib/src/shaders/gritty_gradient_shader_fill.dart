...
class GrittyGradientShaderFill extends StatelessWidget {
  GrittyGradientShaderFill({
    ...
    this.cache = false,
  }) : params = params ?? grittyGradientDef.defaults;
  ...
  static Future<void> precacheShader() =>
      ShaderBuilder.precacheShader('packages/material_palette/shaders/gritty_gradient.frag');

  void _setUniforms(FragmentShader shader, Size size, double time) {
    setShaderUniforms(shader, size, time, params, grittyGradientDef.layout);
  }
  ...
  Widget build(BuildContext context) {
    return ShaderFill(
      width: width,
      height: height,
      shaderPath: 'packages/material_palette/shaders/gritty_gradient.frag',
